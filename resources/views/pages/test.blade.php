<div class="d-flex align-items-center">
    @if (Auth::user()->id == $comment->user->id)
        <svg data-bs-toggle="modal" data-bs-target="#editComment{{ $comment->id }}" xmlns="http://www.w3.org/2000/svg"
    ...
    .5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5z" />
        ...
        <form action="{{ route('comment.delete', $comment->id) }}" method="POST">
            @csrf
            @method('delete')
            <button class="btn" onclick="returnconfirm('Apakah anda yakin ingin menghapus komentar ini?')">
